...
abstract contract InvariantGrowthProtocolFeesWithYield is BaseWeightedPool {
    ...
    function _calculateInvariantWithRates(uint256[] memory rates, uint256[] memory normalizedWeights, uint256[] memory balances)
        internal
        pure
        returns (uint256 invariant)
    {
        ...
        **********************************************************************************************/
        ...
        for (uint256 i = 0; i < normalizedWeights.length; i++) {
            invariant = invariant.mulDown(balances[i].powDown(normalizedWeights[i])).mulDown(rates[i]);
        }

        _require(invariant > 0, Errors.ZERO_INVARIANT);
    }

    // takes extra parameter, rates
    ...
    function _beforeJoinExit(
        ...
        uint256 preJoinExitInvariant = _calculateInvariantWithRates(rates, normalizedWeights, preBalances);
    ...
    function _afterJoinExit(
        ...
        uint256 postJoinExitInvariant = _calculateInvariantWithRates(rates, normalizedWeights, preBalances);
