...
describe('Staking contract', () => {
  ...
  describe('stakeWithPermit', () => {
    ...
    it('stakes with a permit signature', async () => {
      ...
      await stakingContract.connect(other).stakeWithPermit(pool.address, bptBalance, MAX_UINT256, lp.address, lp.address, v, r, s);
    ...
    it('stakes with a permit signature to a recipient', async () => {
      ...
      const { v, r, s } = await signPermit(pool, lp, stakingContract, bptBalance);
      await stakingContract.connect(other).stakeWithPermit(pool.address, bptBalance, MAX_UINT256, lp.address, other.address, v, r, s);
