...
abstract contract TwoTokenPoolsBalance is PoolRegistry {
    ...
    function _registerTwoTokenPoolTokens(
        ...
        _require(poolTokens.tokenA == IERC20(address(uint160(0))) && poolTokens.tokenB == IERC20(address(uint160(0))), Errors.TOKENS_ALREADY_SET);
        ...
        returns (IERC20[] memory tokens, bytes32[] memory balances)
    {
        ...
        // clarity.
        if (tokenA == IERC20(address(uint160(0))) || tokenB == IERC20(address(uint160(0)))) {
    ...
    function _isTwoTokenPoolTokenRegistered(bytes32 poolId, IERC20 token) internal view returns (bool) {
        ...
        return (token == poolTokens.tokenA || token == poolTokens.tokenB) && token != IERC20(address(uint160(0)));
    }
